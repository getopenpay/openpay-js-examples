import { OpenPayForm, SubmitMethods } from '@getopenpay/openpay-js';
import './style.css';
...
function initializeForm(token: string) {
  ...
  const formInstance = new OpenPayForm({
    ...
    onLoad: async (totalAmountAtoms, currency) => {
      ...
      availablePaymentMethods?.forEach((method: any) => {
        ...
        if (method.name === SubmitMethods.airwallexGooglePay && 'isAvailable' in method && method.isAvailable) {
          document.querySelector('#submit-awx-google-pay')?.removeAttribute('disabled');
        }
        if (method.name === SubmitMethods.authorizenetApplePay && 'isAvailable' in method && method.isAvailable) {
          document.querySelector('#submit-authnet-apple-pay')?.removeAttribute('disabled');
        }
        if (method.name === SubmitMethods.authorizenetGooglePay && 'isAvailable' in method && method.isAvailable) {
    ...
    onCheckoutSuccess: (invoiceUrls, subscriptionIds, customerId, processorsUsed) => {
    ...
    onCheckoutError: (error, errorCode) => {
      console.log(`Checkout error [${errorCode}]`, error);
      const errorMessage = {
        type: 'checkout-error',
        message: error,
        errorCode,
  ...
  document.querySelector('#submit-paypal')?.addEventListener('click', () => {
    formInstance.generalSubmit(SubmitMethods.pockytPaypal, {
  ...
  document.querySelector('#submit-awx-google-pay')?.addEventListener('click', () => {
    formInstance.generalSubmit(SubmitMethods.airwallexGooglePay);
  });

  document.querySelector('#submit-awx-apple-pay')?.addEventListener('click', () => {
    formInstance.generalSubmit(SubmitMethods.airwallexApplePay);
  });

  document.querySelector('#submit-authnet-google-pay')?.addEventListener('click', () => {
    formInstance.generalSubmit(SubmitMethods.authorizenetGooglePay);
  });

  document.querySelector('#submit-authnet-apple-pay')?.addEventListener('click', () => {
    formInstance.generalSubmit(SubmitMethods.authorizenetApplePay);
  });
  ...
  function showCheckoutSuccess(
    ...
    if (successElement) {
      ...
      <ul data-testid="processors-used-list" class="text-sm list-inside list-decimal">
        ${processorsUsed.map((processor) => `<li class="mb-2">${processor}</li>`).join('')}
      </ul>
    `;
